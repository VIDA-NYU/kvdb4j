...
public class LevelDBBackend implements KVDB, Closeable {

  protected Options options;
  protected DB db;

  public LevelDBBackend(DB db, Options options) {
    this.db = db;
    this.options = options;
  }

  public static KVDB create(String path) {
    ...
    try {
      ...
      return new LevelDBBackend(db, options);
    } catch (IOException e) {
      String message =
          String.format(
              "Failed to open/create LevelDB database at %s. Error message: %s",
              path, e.getMessage());
  ...
  public BytesBytesIterator createIterator() {
    return new LevelDBIterator(this.db);
  }
}
